...
void kfifo_example(void) {
	...
	t = kfifo_alloc(&queue, 16, GFP_KERNEL); // dinamically allocates a new fifo buffer
	if (t != 0) {
                return;
        }
	//INIT_KFIFO(queue);

	qsize = kfifo_len(&queue); // returns the number of elements used in fifo

	printk(KERN_NOTICE "Queue size is %d", qsize);
	printk(KERN_NOTICE "Inserting into queue");

	kfifo_in(&queue, maintask, sizeof(maintask)); //put data into the fifo
	
	qsize = kfifo_len(&queue);

	if (kfifo_is_empty(&queue)) { //returns true if fifo empty
	...
	{
		kfifo_out(&queue, &val_mytask, sizeof(&val_mytask)); //get data from fifo
		print_task(&val_mytask);
	}

	kfifo_free(&queue); //frees the fifo
